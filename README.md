...
| 系统托盘 | 部分完成 | 已实现托盘常驻图标、设备摘要 tooltip、打开/立即刷新/退出菜单 |
| 任务栏底部展示 | 部分完成 | 已新增实验性任务栏嵌入窗口，展示用户勾选 Bottom 的设备电量；系统托盘 `NotifyIcon` 保留为兜底入口 |
...
- 实验性任务栏嵌入窗口：使用轻量 WPF 窗口挂载到主任务栏，展示用户勾选 Bottom 的设备电量。
...
| System tray | Partially done | Tray icon, battery summary tooltip, and Open/Refresh/Exit menu are available |
| Taskbar bottom display | Partially done | Added an experimental taskbar-embedded WPF window for devices selected with Bottom; the system tray `NotifyIcon` remains as a fallback entry point |
...
- An experimental taskbar-embedded WPF window for selected device battery levels.
