...
| 蓝牙电量读取 | 部分完成 | 已对已连接 BLE 设备尝试读取标准 GATT Battery Service，并对 BTC 设备尝试读取 Windows 暴露的电量属性 |
...
| Bluetooth battery reading | Partially done | Connected BLE devices are read via standard GATT Battery Service when available; BTC devices are attempted through Windows-exposed battery properties |
