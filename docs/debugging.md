...
| 动态扩宽 | 从 1 个设备增加到 2 个设备时，任务栏窗口应立即扩宽，不能出现左侧设备左半边、右侧设备右半边被裁切 |
| 内外尺寸同步 | 任务栏 HWND 变宽时，内部透明根容器和设备列表也应同步变宽，不能只扩大外层窗口 |
| 显式子控件 | 多设备展示应由横向面板中的多个真实子控件组成，不能只渲染最左侧一个设备 |
...
| 设备数量变化后即时重算窗口尺寸 | 已完成，待调试确认 | 监听 `TaskbarOverlayDevices` 变化，并在 WPF 布局完成后重新调用 `SetWindowPos` |
| 内外布局尺寸同步 | 已完成，待调试确认 | 同步设置 Window、根布局和横向设备面板的宽高，避免内层裁剪区域停留在旧尺寸 |
| 显式构建设备小框 | 已完成，待调试确认 | 已用 `StackPanel.Children` 替代 `ItemsControl` 模板，设备数量变化时清空并重建设备小框 |
