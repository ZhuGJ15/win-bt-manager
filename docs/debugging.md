...
| 关闭隐藏 | 点击主窗口关闭按钮后，窗口隐藏，但托盘图标和后台刷新仍保留 |
...
| 关闭隐藏到托盘 | 已完成，待调试确认 | 点击窗口关闭按钮会隐藏窗口并保持后台刷新 |
...
| 检查项 | 预期结果 |
| --- | --- |
| 占位展示 | 应用启动后，任务栏底部区域出现小型 `BT -` 占位，说明任务栏窗口已挂载 |
| Bottom 勾选 | 在主界面勾选某个设备的 Bottom 后，任务栏窗口展示该设备名称、电量百分比和小型电量条 |
...
| 问题 | 处理方式 |
| --- | --- |
| 任务栏没有看到 `BT -` | 确认应用进程仍在运行；如果任务栏启用了自动隐藏、第三方任务栏工具或多显示器扩展任务栏，请先在主显示器、默认底部任务栏布局下验证 |
...
| 电量紧凑展示 | 已完成，待调试确认 | 展示设备名、电量百分比和小型电量条 |
